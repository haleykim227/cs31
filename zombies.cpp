    ...
    switch (dir)
    {
        case 0:
            if ((m_arena->numZombiesAt(m_row - 1, m_col)) != 0)
                m_arena->attackZombieAt(m_row - 1, m_col, 0);
            else if (m_row > 1)
                m_row--;
            break;
        case 1:
            if ((m_arena->numZombiesAt(m_row + 1, m_col)) != 0)
                m_arena->attackZombieAt(m_row + 1, m_col, 1);
            else if (m_row < (m_arena->rows()))
                m_row++;
            break;
        case 2:
            if ((m_arena->numZombiesAt(m_row, m_col - 1)) != 0)
                m_arena->attackZombieAt(m_row, m_col - 1, 2);
            else if (m_col > 1)
                m_col--;
            break;
        case 3:
            if ((m_arena->numZombiesAt(m_row, m_col + 1)) != 0)
                m_arena->attackZombieAt(m_row, m_col + 1, 3);
...
void Arena::display() const
{
    ...
    for (int i = 0; i < rows(); i++)
    {
        ...
        {
            if (numZombiesAt(i+1, j+1) == 1)
                grid[i][j] = 'Z';
            else if ((numZombiesAt(i+1, j+1) > 1) && (numZombiesAt(i+1, j+1) < 9))
                grid[i][j] = ('0' + numZombiesAt(i+1, j+1));
            else if (numZombiesAt(i+1, j+1) >= 9)
                grid[i][j] = '9';
        }
    }
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    for (int i = 0; i < m_nZombies; i++)
    {
        if ((m_zombies[i]->row() == r) && (m_zombies[i]->col() == c))
        {
            if((m_zombies[i]->getAttacked(dir)) == true)
            {
                delete m_zombies[i];
                m_zombies[i] = nullptr;
                m_nZombies--;
...
bool Arena::moveZombies()
{
    ...
    {
        m_zombies[k]->move();
        if (((player()->row()) == (m_zombies[k]->row())) && ((player()->col()) == (m_zombies[k]->col())))
            player()->setDead();
    }
